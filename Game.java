...
public class Game {
	...
	private boolean diagonalWinner() {
		...
		for (int startX = 0; startX < size; startX++) {
			if (checkUpperRightDiagonal(startX, 0)) {
				return true;
			}
		}
	...
	private boolean checkUpperRightDiagonal(int startX, int startY) { // Upper right diagonal win condition fix
		int x = startX;
		int y = startY;
		int counter = 0;
		String currentPlayer = null;
	...
	private void display2dArray(String[][] arr) {
		for (int x = 0; x < arr[0].length; x++) {
			System.out.print(x % 10); // full number display fix
		}
		System.out.println();
	...
	private void makeMove(String move) throws Exception {
		...
		else if (move.compareToIgnoreCase("rot")) { // rot case sensitive fix
			rotateBoard();
		}
		else {
